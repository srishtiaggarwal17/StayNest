...
const HotelCard = ({ room }) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 mb-10 border-b pb-6">
      {/* Hotel Image */}
      <img
        src={
          room.images[0] ||
        ...
        alt="hotel"
        className="w-full sm:w-[300px] h-[200px] object-cover rounded-xl cursor-pointer"
        onClick={() => navigate(`/description/${room._id}`)}
      />
      ...
      <div className="flex flex-col justify-between w-full">
        <div>
          <p className="text-sm text-gray-500">{room.hotel.city}</p>
          <h2 className="text-xl sm:text-2xl font-semibold">{room.hotel.name}</h2>
        ...
        {/* Price */}
        <p className="text-lg sm:text-xl font-bold mt-4 text-gray-800">
          ₹{room.price}
